...
const RULE_FIELD_MAP: Record<string, SheetMapper> = {
  ...
  in_cabin_air_filter_dirty: () => ({ cabinAirFilter: 'replaced' }),
  in_engine_oil_low: () => ({ engineOilLevel: 'sufficient' }),
  in_den_pha_cot_error: () => ({ inputDenPhaCot: 'ok' }),
  in_motor_guong_nut_bam_error: () => ({ inputMotorGuongNutBam: 'ok' }),
  in_cua_so_error: () => ({ inputCuaSo: 'ok' }),
  in_ghe_chinh_dien_error: () => ({ inputGheChinhDien: 'ok' }),
